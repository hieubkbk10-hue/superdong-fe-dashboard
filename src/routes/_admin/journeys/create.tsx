...
import { createJourney } from '@/apis/journeys';
...
function JourneyCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.code) {
      toast.error('Vui lòng nhập Mã Tuyến và Tên Tuyến Hải Trình');
      return;
    }
    ...
    <div className="space-y-6 max-w-4xl font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              Thêm Tuyến Hải Trình Mới
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Thiết lập luồng di chuyển đường biển kết nối bến xuất phát và bến đích
            </p>
          </div>
        </div>
      </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              placeholder="VD: J-RGPQ, J-HTPQ, J-TDCD..."
          ...
          <div>
            ...
            >
              ...
              <option value="Bến tàu Rạch Giá (RG)">Bến tàu Rạch Giá (RG)</option>
              <option value="Bến tàu Nam Du (ND)">Bến tàu Nam Du (ND)</option>
            </select>
          </div>

          <div>
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
              Khoảng Cách Hải Lý (Nautical Miles)
            </label>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="VD: 2 tiếng 30 phút"
                className="w-full h-10 pl-9 pr-3 border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:border-blue-500 outline-none"
              />
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
              Mô Tả &amp; Thông Tin Đặc Thù Tuyến
            </label>
            <textarea
              rows={3}
              value={formData.description}
              onChange={(e) => setFormData({ ...formData, description: e.target.value })}
              placeholder="VD: Tuyến xuyên biển Tây Nam Nam Bộ, chịu ảnh hưởng sóng cấp 4-5 vào mùa gió Nam..."
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
          ...
          >
            <Save size={16} />
            {isSubmitting ? 'Đang lưu...' : 'Lưu Tuyến Mới'}
          </button>
        </div>
      </form>
    </div>
