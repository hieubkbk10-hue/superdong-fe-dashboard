...
import { SeatMap, SeatCell, SeatStatus, SeatClass } from '@/types';
import { BadgeCheck, Luggage, Tv, DoorClosed, Compass, Info, Maximize2, Minimize2, Layers, Sparkles } from 'lucide-react';
...
// Generator for default 275 seat layout grid
function generateDefaultDeckCells(deckLevel: number, rows: number, cols: number): SeatCell[] {
...
const DEFAULT_SEATMAP: SeatMap = {
  id: 'sm-sd-01',
  boat_id: 'boat-01',
  boat_name: 'Superdong Express',
  name: 'Sơ đồ ghế Superdong',
  version: 'v2.1',
  total_seats: 275,
  is_active: true,
  decks: [
    {
      ...
      columns: 8,
      zones: [
        { id: 'zone-vip-1', name: 'Khoang VIP', code: 'VIP-D1', color: 'amber' },
        { id: 'zone-std-1', name: 'Khoang Thường', code: 'STD-D1', color: 'sky' },
      ],
      cells: generateDefaultDeckCells(1, 10, 8)
    },
    {
      id: 'deck-2',
      name: 'Tầng 2 (Khoang thượng)',
      level: 2,
      rows: 8,
      columns: 8,
      zones: [
      ...
      cells: generateDefaultDeckCells(2, 8, 8)
    }
  ]
};

export const SeatMapViewer: React.FC<SeatMapViewerProps> = ({
  seatMap = DEFAULT_SEATMAP,
  onSeatSelect,
  readOnly = false,
  selectedSeatCode,
  className = '',
                      ...
                      <Tooltip key={cell.id || `seat-${idx}`}>
                        ...
                        <TooltipContent className="bg-slate-900 text-slate-100 border-slate-800 p-3 rounded-xl shadow-xl max-w-xs">
                          <div className="space-y-1 text-xs">
                            <div className="font-bold text-cyan-400">Mã ghế: {cell.seat_code}</div>
                          </div>
                        </TooltipContent>
                      </Tooltip>
                    );
          ...
          <div className="border border-border rounded-2xl bg-card p-5 shadow-sm space-y-4 h-fit">
            ...
            {inspectedSeat ? (
              ...
              <div className="py-12 text-center text-muted-foreground space-y-2">
                ...
                <p className="text-xs">Nhấp vào một ghế trên sơ đồ 2D để xem chi tiết vị trí.</p>
              </div>
            )}
          </div>
        </div>
